...
PluginTriggerActions TsneAnalysisPluginFactory::getPluginTriggerActions(const hdps::Datasets& datasets) const
{
    ...
    if (PluginFactory::areAllDatasetsOfTheSameType(datasets, PointType)) {
        if (datasets.count() >= 1) {
            auto pluginTriggerAction = createPluginTriggerAction("Analyze/TSNE analysis", "Perform TSNE analysis on selected datasets", datasets);
        ...
        if (datasets.count() >= 2) {
            auto pluginTriggerAction = createPluginTriggerAction("Group/Analyze/TSNE analysis", "Group datasets and perform TSNE analysis on it", datasets);
